import argparse
import numpy as np
import write_harmonic_pp as harmonic_pp
...
def set_coeff(table_file):
    harmonic_pp.write_file(200.0, 2**(1/6), 1.20)
    cmds = [
        "pair_style     table linear 5000",
        f"pair_coeff     * * {table_file} COS_PP",
    ]
...
def init_velocities(temp, not_random, dimensionality):
    ...
    if dimensionality == 1:
        ...
        return [f"velocity       all create {temp} {seed} dist gaussian",
                ]
...
def run_nvt(w, ensemble, log_name, strain_rate, fix_modify=None):
    halt_condition = [
        ...
        "unfix          1",
        "group          type1 type 1",
        "compute        coord type1 coord/atom cutoff 1.40 group type1",  # for testing: changed from 1.30
    ...
    halt_condition.append(
            "fix            10 all halt 100 v_min_coord < 2 error continue")  # for testing: changed from 20
    ...
    if strain_rate == 0.0:
        ...
        num_steps = np.ceil(final_strain/(0.01*strain_rate))
        num_steps = np.abs(num_steps)

    cmds = [
        f"log            {log_name}",
        # "dump            1 all custom 100 dump.harmonic id x y z fx fy fz",
        f"fix            4 all deform 1 y trate {strain_rate}",
        f"run            {int(num_steps)}",
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--harmonic_filename', type=str)
    ...
    init = [
        ...
        *set_coeff(args.harmonic_filename),
        *set_neighbors(),
        "timestep       0.01",  # for testing: changed from 0.001
    ]
